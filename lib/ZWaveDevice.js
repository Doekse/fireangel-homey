...
/**
 * Reads the notification type from `'Notification Type (Raw)'` (Buffer of one byte) rather than
 * `'Notification Type'`, which `homey-zwavedriver` emits as a localised string label like
 * `'Smoke'` / `'Heat'` / `'System'`.
 *
...
function parseNotificationReport(report) {
  ...
  const raw = report['Notification Type (Raw)'];
  if (!Buffer.isBuffer(raw) || raw.length < 1) return null;
  const type = ZW_NOTIFICATION_TYPE[raw.readUInt8(0)];
  if (!type) return null;
...
class ZWaveDevice extends ZwaveDevice {
  ...
  /**
   * At most one of EOL / inspection / dust is true per update — the device sends one
   * maintenance condition per report.
   *
   * @param {boolean} eol
...
module.exports.parseNotificationReport = parseNotificationReport;
